...
st.set_page_config(page_title="Pyme-Analytics Peru", layout="wide")
...
lista_anos = list(range(2020, 2026))
ano_seleccionado = st.sidebar.selectbox("Seleccione el Ano:", lista_anos, index=len(lista_anos)-1)
...
with tab_demografia:
    ...
    try:
        ...
        if not df_demo.empty:
            ...
            st.line_chart(data=df_demo, x='Mes', y='Stock_Empresas', use_container_width=True)
            
            st.info(f"Interpretacion Demografia: Durante el ano {ano_seleccionado}, el sector {sector_seleccionado} en {region_seleccionada} registro un pico maximo de {df_demo['Natalidad'].max()} empresas nuevas inscritas en un solo mes.")
        else:
...
with tab_laboral:
    ...
    try:
        ...
        if not df_laboral.empty:
            ...
            st.bar_chart(data=df_laboral, x='Mes', y='Poblacion_Ocupada', use_container_width=True)
...
with tab_ventas:
    ...
    try:
        ...
        if not df_ventas.empty:
            ...
            st.bar_chart(data=df_ventas, x='Mes', y='Ventas_Soles', color="#1f77b4", use_container_width=True)
            
            st.info(f"Resumen Estadistico: Las ventas anuales acumuladas en el sector {sector_seleccionado} alcanzaron S/. {total_ventas:,.2f} en el ano {ano_seleccionado}.")
        else:
            st.warning("No se encontraron registros para estos filtros.")
    except Exception as e:
        st.error(f"Error en consulta: {e}")
